...
Do not launch a paper trader from any path unless the latest fresh pre-arm preflight remains current and the exclusive execution window is deliberately armed. The current next operator action is to repair the startup-data gate before another launch attempt.

- Diagnose why `SHOP` stock bars from the configured feed are stale enough to fail startup.
