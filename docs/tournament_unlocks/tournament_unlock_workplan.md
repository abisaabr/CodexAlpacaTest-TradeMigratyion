...
- Generated at: `2026-04-22T15:24:54.950502`
...
- Title: Wire governed execution for `opening_30m_premium_defense`.
- Primary target profile: `opening_30m_premium_defense`
- Action: Add a governed executable entrypoint for `opening_30m_premium_defense` instead of leaving it tracked-only in policy.
...
- Success: `opening_30m_premium_defense` no longer carries `implementation_not_wired` in the unlock registry.
- Success: `opening_30m_premium_defense` is executable in code, even if execution evidence still blocks activation.
...
- `opening_30m_premium_defense`: clear `implementation_not_wired, execution_evidence_floor, broker_order_audit_coverage, broker_activity_audit_coverage` via `wire_profile_entrypoint, upgrade_execution_evidence_to_entry_and_reconciliation, land_trusted_broker_order_audit_sessions, land_trusted_broker_activity_audit_sessions`
