...
## Tonight's Machine Missions

- Research machine: Wire governed execution for `opening_30m_single_vs_multileg`.
...
- Clear the implementation blocker on `opening_30m_single_vs_multileg` so the remaining unlock path is governed only by execution evidence and session trust.
