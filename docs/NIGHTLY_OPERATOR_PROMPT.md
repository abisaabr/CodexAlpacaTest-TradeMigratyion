...
2. refreshes the tournament profile registry and tournament profile handoff
3. refreshes the family registry and handoff packet
4. refreshes the ticker coverage view
5. materializes any missing priority symbols when needed
6. runs the next family discovery wave
7. runs exhaustive follow-up on survivors
8. validates challengers against the current live champion book
9. builds hardening review, replacement plan, and morning handoff packets
10. leaves the live manifest unchanged unless I explicitly approve a reviewed add/replace packet
...
3. Run `build_tournament_profile_registry.py`
4. Run `build_tournament_profile_handoff.py`
5. Run `build_strategy_family_registry.py`
6. Run `build_strategy_family_handoff.py`
7. Run `build_ticker_family_coverage.py`
8. If needed, run `materialize_backtester_ready.py`
9. Build and validate the Phase 1 launch pack
10. Run the discovery lanes
11. Build the shortlist
12. Build and validate the Phase 2 launch pack
13. Run exhaustive follow-up
14. Run shared-account validation
15. Build hardening review
16. Build replacement plan
17. Build morning handoff
18. Refresh the run-registry packet and active-program packet where appropriate
