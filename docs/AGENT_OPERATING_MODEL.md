...
- Wave launchers must treat a lane that exits without `master_summary.json` as a hard failure even if the wrapper process returns exit code `0`; shortlisting and phase handoffs should only consume artifact-backed success.
