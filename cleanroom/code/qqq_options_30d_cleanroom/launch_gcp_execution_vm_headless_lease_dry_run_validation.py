...
DEFAULT_LOCAL_OUTPUT_DIR = REPO_ROOT / "output" / "gcp_execution_vm_headless_lease_dry_run_validation"
DEFAULT_BASE_STARTUP_SCRIPT = textwrap.dedent(
    ...
    cat >/var/lib/codexalpaca/validation_mode.json <<'JSON'
    {
      ...
      "plane": "execution",
      "mode": "paper",
      "stage": "lease_dry_run_validation"
    }
    JSON

    cat >/etc/motd <<'MOTD'
    codexalpaca execution VM
    mode: lease-dry-run-validation-only
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Launch the execution VM lease dry-run validation through a governed headless startup-script run."
    )
...
RUN_ROOT="/var/lib/codexalpaca/lease-validation-launch"
RUN_DIR="$RUN_ROOT/$RUN_ID"
LOG_PATH="/var/log/codexalpaca/lease-validation-$RUN_ID.log"
...
VALIDATION_SCRIPT_PATH="$RUN_DIR/execution_vm_lease_dry_run_validation.sh"
...
lease_status_path = Path("/var/lib/codexalpaca/lease-validation/lease_validation_status.json")
doctor_path = Path("/var/lib/codexalpaca/lease-validation/doctor.json")
pytest_log_path = Path("/var/lib/codexalpaca/lease-validation/pytest_targeted.log")
payload = {{
    "generated_at": datetime.now().astimezone().isoformat(),
    "run_id": run_id,
    "validation_exit_code": int(os.environ["VALIDATION_EXIT"]),
    "lease_validation_status_present": lease_status_path.exists(),
    "doctor_present": doctor_path.exists(),
    "pytest_log_present": pytest_log_path.exists(),
    "log_path": os.environ["LOG_PATH"],
}}
if lease_status_path.exists():
    try:
        payload["lease_validation_status"] = json.loads(lease_status_path.read_text(encoding="utf-8"))
    except Exception as exc:  # noqa: BLE001
        payload["lease_validation_status_read_error"] = str(exc)
...
export LOCAL_PATH="/var/lib/codexalpaca/lease-validation/lease_validation_status.json"
export UPLOAD_GCS_URI="$BASE_GCS_PREFIX/lease_validation_status.json"
upload_file "$LOCAL_PATH" "$UPLOAD_GCS_URI"

export LOCAL_PATH="/var/lib/codexalpaca/lease-validation/doctor.json"
export UPLOAD_GCS_URI="$BASE_GCS_PREFIX/doctor.json"
upload_file "$LOCAL_PATH" "$UPLOAD_GCS_URI"

export LOCAL_PATH="/var/lib/codexalpaca/lease-validation/pytest_targeted.log"
export UPLOAD_GCS_URI="$BASE_GCS_PREFIX/pytest_targeted.log"
upload_file "$LOCAL_PATH" "$UPLOAD_GCS_URI"

export LOCAL_PATH="/var/lib/codexalpaca/lease-validation/lease_validation.log"
export UPLOAD_GCS_URI="$BASE_GCS_PREFIX/lease_validation.log"
upload_file "$LOCAL_PATH" "$UPLOAD_GCS_URI"
...
def is_headless_composite_script(text: str) -> bool:
    return "RUN_ROOT=\"/var/lib/codexalpaca/lease-validation-launch\"" in text
...
def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        ...
        "## Audit Objects",
        "",
        f"- Composite startup script (local): `{payload['local_startup_script_path']}`",
        f"- Composite startup script (GCS): `{payload['startup_script_gs_uri']}`",
        f"- Validation result prefix: `{payload['validation_result_gcs_prefix']}`",
        "",
        "## Trigger",
        "",
        f"- Metadata operation: `{payload['metadata_operation_name']}`",
        f"- Reset operation: `{payload['reset_operation_name']}`",
        "",
        "## Follow-Up",
        "",
    ]
...
def main() -> None:
    ...
    validation_status_path = report_dir / "gcp_execution_vm_lease_dry_run_validation_status.json"
    ...
    control_prefix = f"bootstrap/{args.date_prefix}/foundation-phase17-lease-dry-run-validation"
    ...
    for item in items:
        ...
        if key == "startup-script":
            ...
            continue
        if key in {
            "codexalpaca-lease-validation-run-id",
            "codexalpaca-lease-validation-script-gs-uri",
            "codexalpaca-lease-validation-control-prefix",
            "codexalpaca-lease-validation-launch-mode",
        }:
            continue
    ...
    startup_script_name = (
        f"execution_vm_headless_lease_dry_run_validation_startup_{args.vm_name}_{run_id}.sh"
    )
    ...
    updated_items = other_items + [
        ...
        {"key": "codexalpaca-lease-validation-run-id", "value": run_id},
        {"key": "codexalpaca-lease-validation-script-gs-uri", "value": validation_script_gs_uri},
        {
            "key": "codexalpaca-lease-validation-control-prefix",
            "value": f"gs://{args.control_bucket}/{control_prefix}/{run_id}",
        },
        {"key": "codexalpaca-lease-validation-launch-mode", "value": "headless_startup"},
    ]
    ...
    payload = {
        ...
        "zone": args.zone,
        "run_id": run_id,
        "launch_state": "headless_lease_dry_run_validation_triggered",
        ...
        "next_actions": [
            ...
            f"Inspect the GCS result prefix at gs://{args.control_bucket}/{control_prefix}/{run_id} for launch_result.json, lease_validation_status.json, doctor.json, pytest_targeted.log, lease_validation.log, and validation-run.log.",
            "Only if the headless lease dry-run review passes should the next step move toward broker-facing trusted validation.",
        ],
    }

    json_path = report_dir / "gcp_execution_vm_headless_lease_dry_run_validation_status.json"
    md_path = report_dir / "gcp_execution_vm_headless_lease_dry_run_validation_status.md"
