...
def test_executor_readiness_accepts_powershell_bom_json(tmp_path: Path) -> None:
    runner = tmp_path / "runner"
    for relative in [
        "scripts/build_historical_dataset.py",
        "scripts/run_sample_backtest.py",
    ...
    smoke_json.write_text(json.dumps(smoke_payload), encoding="utf-8-sig")
    ...
    assert payload["status"] == "ready_for_research_only_execution_smoke_validated"
    assert payload["smoke_run_proof"]["run_id"] == "powershell_manifest"
